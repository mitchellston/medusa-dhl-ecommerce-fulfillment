...
export const GET = async (
  ...
  try {
    ...
    const record = await service.retrievePricingManual(id)
    res.json({ record: record as PricingRecord })
  } catch (error) {
    console.error('Error retrieving pricing manual:', error)
...
export const POST = async (
  ...
  try {
    ...
    const record = await service.updatePricingManuals({
      id,
      ...rest,
    ...
    console.error('Error updating pricing manual:', error)
...
export const DELETE = async (
  ...
  try {
    const { id } = req.params
    const service = req.scope.resolve(DHL_SETTINGS_MODULE) as DHLSettingsModuleService
    await service.deletePricingManuals([id])
    res.json({ success: true })
  } catch (error) {
    console.error('Error deleting pricing manual:', error)
    res.status(500).json({ message: 'Internal server error' })
  }
}
