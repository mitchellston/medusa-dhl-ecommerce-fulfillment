...
export const GET = async (
  ...
  try {
    ...
    const [records, count] = await service.listAndCountPricingManuals(filters)

    res.json({ records: records as PricingRecord[], count })
  } catch (error) {
    console.error('Error listing pricing manuals:', error)
...
export const POST = async (
  ...
  try {
    ...
    const record = await service.createPricingManuals({
      ...rest,
    ...
    console.error('Error creating pricing manual:', error)
    res.status(500).json({ message: 'Internal server error' })
  }
}
