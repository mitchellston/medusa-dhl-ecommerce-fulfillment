...
export default async function handler({ container }: { container: MedusaContainer }) {
  const dhlSettingService = container.resolve(DHL_SETTINGS_MODULE) as DHLSettingsModuleService
